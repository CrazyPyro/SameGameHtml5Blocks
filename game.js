    ...
    return {
      ...
      computeDestroy: function(x,y) {
        ...
        var recCompute = function(x, y, numFilter) {
          console.log("recCompute("+x+","+y+","+numFilter+")");
    ...
    return {
      ...
      drawHover: function(points) {
        ...
        for(var p in points) {
          ...
          ctx.rect(points[p].x, 0.5+gridSize.h-1-points[p].y, gridSize.w, gridSize.h);
          ctx.fill();
        }
        ctx.restore();
      }
...
/// game.Canvas ///
  ...
  game.Canvas = function() {
    ...
    var drawBrick = function(x, y) {
      ...
      ctx.beginPath();
      //ctx.arc(0.5, 0.5, 0.4, 0, Math.PI*2, true);
      ctx.rect(0,0,1,1);
      ctx.fill();
      ctx.restore();
    };
    ...
    var updateWindowSize = function() {
      ...
      canvas.height = $(window).height()-$('#optionsContainer').height()-42;
